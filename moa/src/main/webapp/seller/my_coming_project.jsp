...
<%
	...
	List<ProjectDto> list = projectDao.comingSelectList(p, s, sellerNo);
%>
...
<jsp:include page = "/template/header.jsp"></jsp:include>
	...
	<div class="row m30"><hr></div>
			...
			<div class="container mt30">
                    ...
                    <div class="float-container">
                        ...
                        <div class="float-left layer-5 p10">
                            <a href="<%=request.getContextPath() %>/seller/my_coming_project.jsp" class="link link-btn fill center">공개 예정</a>
                        </div>
                        <div class="float-left layer-5 p10">
	...
	<div class="container">
		<!-- 페이지네이션 -->
		<%
		int count = projectDao.comingCountByPaging(sellerNo);
		...
		<h4>
			...
			<div class="pagination center">
				...
				if (p > 1) { // 첫페이지가 아니라면
				%>
					<a href="my_coming_project.jsp?p=1&s=<%=s%>">&laquo;</a>
				<%}%>

				<%
				if (startBlock > 1) { // 이전 블록이 있으면
				%>
					<a href="my_coming_project.jsp?p=<%=startBlock - 1%>&s=<%=s%>">&lt;</a>
				<%}%>
				...
				<%for (int i = startBlock; i <= endBlock; i++) {%>
					<%if (i == p) {%>
						<a class="active" href="my_coming_project.jsp?p=<%=i%>&s=<%=s%>"><%=i%></a>
					<%} else {%>
						<a href="my_coming_project.jsp?p=<%=i%>&s=<%=s%>"><%=i%></a>
					<%} %>
				<%}%>

				<!-- 다음 버튼 영역 -->
				<%if (endBlock < lastPage) {%>
					<a href="my_coming_project.jsp?p=<%=endBlock + 1%>&s=<%=s%>">&gt;</a>
				<%}%>

				<%
				if (p < lastPage) { // 마지막 페이지가 아니라면
				%>
					<a href="my_coming_project.jsp?p=<%=lastPage%>&s=<%=s%>">&raquo;</a>
				<%}%>
			</div>
