...
<%-- 
	진행중인 펀딩목록
	default가 될 페이지로 펀딩을 누르면 이 목록이 뜬다.
--%>
...
<body>
	...
	<div class="container w1000">
		...
		<%-- 카테고리 선택창 --%>
		<a href="ongoingProjectList.jsp">전체</a> <a
			href="ongoingProjectList.jsp?type=project_category&keyword=패션/잡화">패션/잡화</a>
		<a href="ongoingProjectList.jsp?type=project_category&keyword=뷰티">뷰티</a> <a
			href="ongoingProjectList.jsp?type=project_category&keyword=푸드">푸드</a> <a
			href="ongoingProjectList.jsp?type=project_category&keyword=홈/리빙">홈/리빙</a> <a
			href="ongoingProjectList.jsp?type=project_category&keyword=테크/가전">테크/가전</a>
		<a href="ongoingProjectList.jsp?type=project_category&keyword=기타">기타</a>
		...
		if (isSearch) {
		%>
		<form action="ongoingProjectList.jsp" method="get">
		...
		<%-- <div class="row center">
			...
			<table class="table table-border table-hover">
				...
				<tbody align="center">
					...
					<tr>
						<td><%=projectDto.getProjectNo()%></td>
						<td><%=projectDto.getProjectName()%></td>
						<td><%=projectDto.getProjectCategory()%></td>
						<td><%=projectDto.getProjectSummary()%></td>
						<td><%=projectDto.getProjectStartDate()%></td>
						<td><%=projectDto.getProjectFinishDate()%></td>
					</tr>
					<%}%>
				</tbody>
		...
		<div class="container m30">
		 ...
		 <%for(ProjectDto projectDto : list) { %>
            <div class="a">
              <div class="row center">
                <img src="download.kh?projectNo=<%=projectDto.getProjectNo() %>" width="100%">
              </div>
              <div class="row name"><%=projectDto.getProjectName() %></div>
