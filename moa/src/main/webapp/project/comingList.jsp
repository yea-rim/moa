...
<jsp:include page="/template/header.jsp"></jsp:include>
	...
	<div class="container">
		...
		<%if(isExist){ %>
		 ...
		 <%for(ProjectDto projectDto : list) { %>
            ...
            <div class="flex-items">
              ...
              <div class="row project-name3"><%=projectDto.getProjectName() %></div>
              <div class="row">
		...
		int count;
		if (isSearch) {
			count = projectDao.comingCountByPaging(type, keyword);
		} else {
			count = projectDao.comingCountByPaging();
		}
		...
		<h4>
			...
			<div class="pagination center">
				...
				<%if (isSearch) {%>
					<%if(sort != null) { %>
					<a href="comingList.jsp?p=1&s=<%=s%>&type=<%=type%>&keyword=<%=keyword%>&sort=<%=sort %>">&laquo;</a>
					<%} else{ %>
					<a href="comingList.jsp?p=1&s=<%=s%>&type=<%=type%>&keyword=<%=keyword%>">&laquo;</a>
					<%} %>
				<%} else {%>
				<a href="comingList.jsp?p=1&s=<%=s%>&sort=<%=sort %>">&laquo;</a>
				<%}%>
				<%}%>
				...
				<%if (isSearch) {%>
				<a
					href="comingList.jsp?p=<%=startBlock - 1%>&s=<%=s%>&type=<%=type%>&keyword=<%=keyword%>">&lt;</a>
				<%} else {%>
				<a href="comingList.jsp?p=<%=startBlock - 1%>&s=<%=s%>">&lt;</a>
				<%}%>
				<%}%>
				...
				<%if (isSearch) {%>
					<%if(sort != null) {%>
						<%if (i == p) {%>
						<a class="active" href="comingList.jsp?p=<%=i%>&s=<%=s%>&type=<%=type%>&keyword=<%=keyword%>&sort=<%=sort %>"><%=i%></a>
						<%} else {%>
						<a href="comingList.jsp?p=<%=i%>&s=<%=s%>&type=<%=type%>&keyword=<%=keyword%>&sort=<%=sort %>"><%=i%></a>
						<%}%>
					<%} else { %>
						<%if (i == p) {%>
						<a class="active" href="comingList.jsp?p=<%=i%>&s=<%=s%>&type=<%=type%>&keyword=<%=keyword%>"><%=i%></a>
						<%} else {%>
						<a href="comingList.jsp?p=<%=i%>&s=<%=s%>&type=<%=type%>&keyword=<%=keyword%>"><%=i%></a>
						<%}%>
					<%} %>

				<%} else {%>
				<%if (i == p) {%>
				<a class="active" href="comingList.jsp?p=<%=i%>&s=<%=s%>&sort=<%=sort %>"><%=i%></a>
				<%} else {%>
				<a href="comingList.jsp?p=<%=i%>&s=<%=s%>&sort=<%=sort %>"><%=i%></a>
				<%}%>
				...
				<%if (isSearch) { // 다음 블록이 있으면 %>
					<%if(sort != null){ %>
					<a href="comingList.jsp?p=<%=endBlock + 1%>&s=<%=s%>&type=<%=type%>&keyword=<%=keyword%>&sort<%=sort %>">&gt;</a>
					<%} else { %>
					<a href="comingList.jsp?p=<%=endBlock + 1%>&s=<%=s%>&type=<%=type%>&keyword=<%=keyword%>">&gt;</a>
					<% } %>
				<%} else {%>
				<a href="comingList.jsp?p=<%=endBlock + 1%>&s=<%=s%>&sort=<%=sort %>">&gt;</a>
				<%}%>
				<%}%>
				...
				<%if (isSearch) {%>
					<%if(sort != null) { %>
					<a href="comingList.jsp?p=<%=lastPage%>&s=<%=s%>&type=<%=type%>&keyword=<%=keyword%>&sort=<%=sort %>">&raquo;</a>
					<%} else {%>
					<a href="comingList.jsp?p=<%=lastPage%>&s=<%=s%>&type=<%=type%>&keyword=<%=keyword%>">&raquo;</a>
					<%} %>
				<%} else {%>
				<a href="comingList.jsp?p=<%=lastPage%>&s=<%=s%>&sort=<%=sort %>">&raquo;</a>
				<%}%>
				<%}%>
			</div>
